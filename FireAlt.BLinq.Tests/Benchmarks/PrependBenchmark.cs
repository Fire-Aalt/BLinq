...
namespace FireAlt.BLinq.Tests.Benchmarks
{
    ...
    public class PrependBenchmark : IBenchmark
    {
        ...
        {
            return values.Where(Include).Prepend(17).Sum(Select);
        }

        public int ZLinq(in NativeArray<int> values)
        {
            return values.AsValueEnumerable().Where(Include).Prepend(17).Sum(Select);
        }

        public static int BLinq(in NativeArray<int> values)
        {
            return values.AsQuery().Where(Include).Prepend(17).Sum(Select);
        }
