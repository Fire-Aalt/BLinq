namespace FireAlt.BLinq.Generators
{
    ...
    public sealed class QueryExtensionGenerator : IIncrementalGenerator
    {
        ...
        private static string GenerateSource(ImmutableArray<QueryExtensionData> queryExtensions)
        {
            ...
            {
                namespaceWriter.Block("public static partial class BLinqExtensions", classWriter =>
                {
                    for (var i = 0; i < queryExtensions.Length; i++)
                    {
                        ...
                        AppendQueryExtension(classWriter, queryExtensions[i]);
                    }
                });
            });
        ...
        private static void AppendQueryExtension(SourceWriter source, QueryExtensionData queryExtension)
        {
            ...
            AppendAggressiveInlining(source);
            source.Line($"public static {queryType} AsQuery{typeParameterList}(this {queryExtension.CollectionTypeName} collection)");
            source.Indent();
            foreach (var constraintClause in queryExtension.ConstraintClauses)
            {
                source.Line(constraintClause);
            }

            source.Unindent();
            source.Block(methodWriter =>
            {
                methodWriter.Line($"return new {queryType}({queryExtension.EnumeratorExpression});");
            });
        }
