namespace FireAlt.BLinq.Generators
{
    ...
    public sealed class AccumulatorGenerator : IIncrementalGenerator
    {
        ...
        private static string GenerateSource(AccumulatorData accumulator)
        {
            ...
            AppendHeader(source);
            source.Block("namespace FireAlt.BLinq", namespaceWriter =>
            {
        ...
        private static void AppendAccumulator(SourceWriter source, AccumulatorData accumulator)
        {
            ...
            source.Block($"public struct {accumulator.StructName} : global::FireAlt.BLinq.IAccumulator<{accumulator.FullTypeName}>", structWriter =>
            {
        ...
        private static void AppendSumAverageExtensions(SourceWriter source, AccumulatorData accumulator)
        {
            source.Block("public static partial class BLinqExtensions", classWriter =>
            {
                ...
                {
                    methodWriter.Line($"return source.Sum<{accumulator.FullTypeName}, TEnumerator, global::FireAlt.BLinq.{accumulator.StructName}>();");
                });
                ...
                classWriter.Line($"where TSelector : unmanaged, global::FireAlt.BLinq.ISelector<TSource, {accumulator.FullTypeName}>");
                classWriter.Unindent();
                classWriter.Block(methodWriter =>
                {
                    methodWriter.Line($"return source.Sum<TSource, {accumulator.FullTypeName}, TEnumerator, TSelector, global::FireAlt.BLinq.{accumulator.StructName}>(selector);");
                });
                ...
                classWriter.Block(methodWriter =>
                {
                    methodWriter.Line($"return source.Average<{accumulator.FullTypeName}, TEnumerator, global::FireAlt.BLinq.{accumulator.StructName}>();");
                });
                ...
                classWriter.Block(methodWriter =>
                {
                    methodWriter.Line($"return source.Average<TSource, {accumulator.FullTypeName}, TEnumerator, TSelector, global::FireAlt.BLinq.{accumulator.StructName}>(selector);");
                });
            });
        }
